...
function errorMessage(err: unknown) {
  return err instanceof Error ? err.message : 'Algo salió mal. Probá de nuevo.';
}
...
export function EditableTable<T extends { id: string }>({
  ...
  function commitEdit(rowId: string, col: ColumnDef<T>, raw: string) {
    ...
    startTransition(() => {
      onUpdate(rowId, patch).catch((err) => setActionError(errorMessage(err)));
    });
  }
  ...
  function handleDelete(id: string) {
    ...
    startTransition(() => {
      onDelete(id).catch((err) => setActionError(errorMessage(err)));
    });
  }

  function submitNewRow() {
    const primaryCol = columns[0];
    if (!newRow[primaryCol.key]?.trim()) {
      setFormError(`Completá "${primaryCol.label}" para poder agregar.`);
      return;
    }
    setFormError(null);
  ...
  return (
    <div>
      {actionError && (
        <div className="mb-3 flex items-center justify-between gap-3 rounded-lg border border-danger/30 bg-accent-soft px-3 py-2 text-sm text-danger">
          <span>{actionError}</span>
          <button onClick={() => setActionError(null)} className="shrink-0 text-xs underline">
            Cerrar
          </button>
        </div>
      )}
        ...
        <div className="scroll-thin overflow-x-auto">
          <table className="w-full min-w-[760px] border-collapse text-sm">
            ...
            <tfoot>
              ...
              {formError && (
                <tr>
                  <td colSpan={columns.length + 1} className="px-4 pb-3 text-xs text-danger">
                    {formError}
                  </td>
                </tr>
              )}
